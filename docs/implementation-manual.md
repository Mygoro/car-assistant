...
> 하고 맥락이 없다"는 한계가 드러나 신설했다. 예: 노션에서 여자친구 데이트 맥락이
> 나와도 그걸 장소 추천에 엮지 못하고, 주유소 5개를 나열만 할 뿐 무엇을 왜 추천하는지
> 특화된 판단이 없었다(구 이슈 5). 또 사용자 정보(이름·차량·자주 가는 곳·말투)를 매
...
1. **`core/memory.md` 확장 — 상시 주입되는 사용자 컨텍스트**
   - 사용자 프로필: 이름, 차량(아반떼), 자주 가는 목적지, 반복 맥락(여자친구 등),
     선호. 매 세션 다시 입력하지 않게 한다.
...
3. **맥락 통합(구 이슈 5)** — 앞선 도구 결과나 노션 맥락을 후속 검색·추천에 반영.
   "여자친구 데이트" 맥락 → 장소 추천 시 분위기·동선 고려. 단순 나열이 아니라
   "그중 무엇을 왜 추천하는지"를 답하게 한다.

### 품질 검증

- [ ] 사용자 이름·차량·자주 가는 곳을 다시 알려주지 않아도 답에 반영된다
- [ ] "여자친구랑 갈 만한 곳" → 노션 맥락 + Places 평점/영업시간을 엮어 특화 추천한다
- [ ] 같은 질문에 소스 선택이 일관된다(영업시간은 Places, 위치는 카카오 등)
- [ ] 복합 질의("기름 넣고 밥 먹게 근처 추천")에서 차량+장소+상세를 순서대로 조합한다
...
1. **히스토리 요약 압축** — 차량 세션이 길어지면 토큰·환각이 늘어난다. 오래된 대화를
   요약해 컨텍스트를 압축(에이닷이 향후 과제로 꼽은 것).
2. **tool 결과 코드 검증층** — 프롬프트 환각 방지(이번 세션 적용) 위에, 모델이 응답에
   쓴 장소명·수치가 실제 API 결과 목록에 있는지 **코드로 대조**하는 후처리.
3. **STT 2차 보정** — 프롬프트 보정(적용됨) 위에, Whisper `initial_prompt`에 자주 쓰는
   고유명사(목적지·도구명)를 주입해 인식 단계부터 잡는다.
4. **모호한 발화의 맥락 명확화** — "거기로 가자" → "방금 검색한 ○○ 말씀이죠?"처럼
   직전 맥락으로 되묻기보다 명확화(에이닷 목표 수준).
5. **회귀 테스트 + 품질 기준** — 핵심 시나리오 묶음을 정해 매 변경 후 재실행.

### 품질 검증

- [ ] 긴 세션(20+ 턴)에서도 토큰이 폭주하지 않고 과거 맥락을 유지한다
- [ ] 모델이 API에 없는 장소·수치를 답하면 코드 검증층이 잡아낸다
- [ ] 자주 쓰는 고유명사가 STT 단계에서 안정적으로 인식된다
- [ ] 핵심 시나리오 회귀 묶음이 변경 후에도 통과한다
